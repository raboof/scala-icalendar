...
class PropertyParametersSpec extends AnyWordSpec with Matchers {
  "3.2 Property Parameters" should {
    ...
    "3.2.2 Common Name" in {
      asIcal(
        Organizer(CalAddress("mailto:jsmith@example.com", Cn("John Smith")))
      ) should haveLines("ORGANIZER;CN=John Smith:mailto:jsmith@example.com")
    }

    "3.2.3 Calendar User Type" in {
      asIcal(
        Attendee(CalAddress("mailto:ietf-calsch@example.org", cutype = Group))
      ) should haveLines("ATTENDEE;CUTYPE=GROUP:mailto:ietf-calsch@example.org")
    }
      ...
      asIcal(
        Attendee(
          CalAddress(
            ...
            delegatedFrom =
              DelegatedFrom(List(CalAddress("mailto:jsmith@example.com")))
          )
        )
      ) should haveLines(
    ...
    "3.2.10 Language" in {
      ...
      ) should haveLines("SUMMARY;LANGUAGE=en-US:Company Holiday Party")

      asIcal(Location("Tyskland", language = Language("no"))) should haveLines(
    ...
    "3.2.11 Group or List Membership" in {
      asIcal(
        Attendee(
          CalAddress(
            "mailto:jsmith@example.com",
            member = Member(List(CalAddress("mailto:ietf-calsch@example.org")))
          )
        )
      ) should haveLines(
    ...
    "3.2.20 Value Data Types" in {
      asIcal(Dtend(Date(LocalDate.parse("1998-07-04")))) should

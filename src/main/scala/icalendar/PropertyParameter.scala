...
object PropertyParameters:
  ...
  case class ExperimentalCuType(xname: Xname)
      extends CutypeValue
      with XnameValue
  case class IanaCuType(token: IanaToken)
      extends CutypeValue
      with IanaTokenValue

  case class DelegatedFrom(value: List[CalAddress])
      extends PropertyParameter[List[CalAddress]]
  case class DelegatedTo(value: List[CalAddress])
      extends PropertyParameter[List[CalAddress]]
  ...
  case class ExperimentalFbtype(xname: Xname)
      extends FbtypeValue
      with XnameValue
  case class IanaFbtype(token: IanaToken)
      extends FbtypeValue
      with IanaTokenValue
